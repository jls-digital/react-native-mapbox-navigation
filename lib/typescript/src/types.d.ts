 ...
 */
export type MapboxLanguage = 'ar' | 'ca' | 'cs' | 'da' | 'de' | 'en' | 'es' | 'fi' | 'fr' | 'he' | 'hu' | 'id' | 'it' | 'ja' | 'ko' | 'nb' | 'nl' | 'pl' | 'pt-BR' | 'pt-PT' | 'ro' | 'ru' | 'sl' | 'sv' | 'tr' | 'uk' | 'vi' | 'zh-Hans';
/**
 * Machine-readable error code emitted via {@link MapboxNavigationProps.onError}.
 * See SPEC §7 for the meaning of each code.
 */
export type MapboxNavigationErrorCode = 'ROUTE_CALCULATION_FAILED' | 'GPS_UNAVAILABLE' | 'GPS_PERMISSION_DENIED' | 'NETWORK_ERROR' | 'INVALID_COORDINATES' | 'SDK_INIT_FAILED';
