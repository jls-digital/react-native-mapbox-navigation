...
Fabric native component via Nitro Modules (`react-native-nitro-modules`). The `.nitro.ts` spec file is the single source of truth for the bridge layer. No old architecture (`UIManager`, `NativeModules`, `DeviceEventEmitter`) code.

### T2 — Nitro Spec

Define a `.nitro.ts` spec file with:

- `HybridViewProps` extension with typed props for all configuration
- Callback props for all events (Nitro handles JS ↔ native function bridging via JSI)
- `HybridViewMethods` extension for imperative operations
- The `nitrogen` CLI generates native bindings from this spec

### T3 — Imperative Commands

Expose via `HybridViewMethods` on the Nitro view ref (`MapboxNavigationRef`):
...
- iOS: **Swift** (Nitro Modules supports Swift directly — no ObjC++ interop needed)
- Android: **Kotlin**
...
Use `create-react-native-library` with the `nitro-view` template as the project scaffold.
...
└──────────────────────┬──────────────────────────┘
                       │ Fabric / Nitro Modules
┌──────────────────────┴──────────────────────────┐
│              react-native-mapbox-navigation      │
│                                                  │
│  src/                                            │
│  ├── ReactNativeMapboxNavigation.nitro.ts        │
│  │   (Nitro spec — props, events, methods)       │
│  ├── MapboxNavigation.tsx                        │
│  │   (React wrapper with ref forwarding)         │
│  └── types.ts                                    │
│                                                  │
│  nitrogen/generated/                             │
│  │   (auto-generated native bindings — do not    │
│  │    edit; regenerated by `nitrogen` CLI)        │
│  │                                               │
│  ios/                                            │
│  ├── ReactNativeMapboxNavigation.swift           │
│  │   (HybridView wrapping NavigationViewController│
│  │    from SDK — pure Swift, no ObjC++ needed)   │
│  │                                               │
│  android/                                        │
│  ├── ReactNativeMapboxNavigation.kt              │
│  │   (HybridView wrapping Mapbox NavigationView) │
└──────────────────────┬──────────────────────────┘
                       │ Native SDK
┌──────────────────────┴──────────────────────────┐
